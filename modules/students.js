...
function renderTableFromData(data) {
  ...
  tbody.innerHTML = data.map(s => {
    ...
    return `
      <tr style="${rowStyle}">
        <td>
          ${s.child_name}
          ${isLowBalance ? `<span class="low-balance-icon" data-id="${s.id}" style="cursor: pointer; margin-left: 0.5rem;" title="Напомнить об оплате">⚠️</span>` : ''}
          ${isDebt ? `<span class="debt-icon" data-id="${s.id}" style="cursor: pointer; margin-left: 0.5rem;" title="Долг">🔴</span>` : ''}
        </td>
        <td>${s.parent_name || '—'}</td>
  ...
  document.querySelectorAll('.low-balance-icon').forEach(icon => {
    icon.addEventListener('click', (e) => {
      e.stopPropagation();
      openStudentCard(icon.dataset.id, 'reminder');
    });
  });
  document.querySelectorAll('.debt-icon').forEach(icon => {
    icon.addEventListener('click', (e) => {
      e.stopPropagation();
      openStudentCard(icon.dataset.id, 'payments');
    });
  });
}

export function resetStudentsCache() {
  setPageCached('students', false);
...
export async function openStudentCard(studentId, activeTab = 'report') {
  ...
  modal.innerHTML = `
    <div class="modal-card" style="max-width: 750px;">
      ...
      <div class="tabs">
        ...
        <button class="tab" data-tab="reminder">🔔 Напомнить</button>
      </div>
      <div class="tab-content" id="studentInfoTab">${renderStudentInfo(student, balance)}</div>
      <div class="tab-content" id="studentPaymentsTab">${await renderPaymentsTab(studentId, paymentsData, totalPaidLessons)}</div>
      <div class="tab-content" id="studentLessonsTab">${renderLessonsTab(lessonsData, usedPaymentsData)}</div>
      <div class="tab-content" id="studentReportTab">${await renderReportTab(student, lessonsData, paymentsData)}</div>
  ...
  tabs.forEach(tab => {
    if (tab.dataset.tab === activeTab) {
      ...
      modal.querySelector(`#student${activeTab.charAt(0).toUpperCase() + activeTab.slice(1)}Tab`).classList.add('active');
    } else {
...
`;
    if (filteredLessons.length > 0) {
      filteredLessons.forEach(l => { reportText += `• ${new Date(l.lesson_date).toLocaleDateString('ru-RU')} – ${l.topic || 'без темы'}\n`; });
    } else {
...
`;
    ...
    if (upcomingLessons.length > 0) {
      upcomingLessons.forEach(l => { reportText += `• ${new Date(l.lesson_date).toLocaleString('ru-RU', { day: 'numeric', month: 'long', hour: '2-digit', minute: '2-digit' })} – ${l.topic || 'без темы'}\n`; });
    } else {
...
SimpleEd CRM`;
  ...
  modal.querySelectorAll('.quick-period').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      switch (period) {
        ...
        case 'month': start = new Date(now); start.setDate(now.getDate() - 30); end = now; break;
        case '3lessons': case '5lessons':
          const count = period === '3lessons' ? 3 : 5;
          const completed = lessonsData.filter(l => l.status === 'completed' || l.attended === true).sort((a, b) => new Date(b.lesson_date) - new Date(a.lesson_date)).slice(0, count);
          if (completed.length > 0) { start = new Date(completed[completed.length - 1].lesson_date); end = new Date(completed[0].lesson_date); }
          else { start = now; end = now; }
          break;
      }
  ...
  updateReport();
  modal.querySelector('#copyReportBtn')?.addEventListener('click', () => { navigator.clipboard.writeText(previewDiv.innerText); alert('Отчёт скопирован!'); });
  ...
  modal.querySelector('#aiRecommendBtn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      const filteredLessons = lessonsData.filter(l => (l.status === 'completed' || l.attended === true) && l.lesson_date >= start && l.lesson_date <= end);
      const stats = { completed: filteredLessons.length, missed: lessonsData.filter(l => l.status === 'cancelled' && l.lesson_date >= start && l.lesson_date <= end).length };
      ...
      const response = await fetch('https://yyohojhvayfcwiqrdiqf.supabase.co/functions/v1/super-function', {
        ...
        body: JSON.stringify({ student: { child_name: student.child_name, child_age: student.child_age }, period: { start, end }, stats, lessons: filteredLessons.map(l => ({ topic: l.topic })), notes })
      });
  ...
  // Обработчики для вкладки "Напомнить"
  const nextTopicsInput = modal.querySelector('#nextTopics');
  const reminderTextarea = modal.querySelector('#reminderMessage');
  modal.querySelector('#copyReminderBtn')?.addEventListener('click', () => { navigator.clipboard.writeText(reminderTextarea.value); alert('Сообщение скопировано!'); });
  modal.querySelector('#generateAIMessageBtn')?.addEventListener('click', async () => {
    const btn = modal.querySelector('#generateAIMessageBtn');
    btn.disabled = true;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Генерация...';
    try {
      const completed = lessonsData.filter(l => l.status === 'completed' || l.attended === true);
...
async function renderPaymentsTab(studentId, payments, totalPaidLessons) {
  let html = `<div style="padding: 1rem;"><div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;"><div><strong>Всего оплачено уроков:</strong> <span style="font-size:1.2rem;">${totalPaidLessons}</span></div><button class="btn btn-primary btn-sm" id="addPaymentBtn"><i class="fas fa-plus"></i> Добавить платёж</button></div><table style="width:100%;"><thead><tr><th>Дата</th><th>Сумма</th><th>Уроков</th><th>Период</th><th>Заметка</th><th>Статус</th></tr></thead><tbody>`;
  if (payments.length) {
    payments.forEach(p => {
      const period = p.period_start ? `${p.period_start} – ${p.period_end}` : '—';
      html += `<tr><td>${p.payment_date}</td><td>${p.amount ? p.amount + ' ₽' : '—'}</td><td>${p.lessons_paid || '—'}</td><td>${period}</td><td>${p.description || '—'}</td><td>${p.status === 'paid' ? '✅' : '❌'}</td></tr>`;
    });
  } else {
  ...
  if (lessons && lessons.length) {
    lessons.forEach(l => {
      ...
      if (l.is_free) { spentInfo = '🎁 Бесплатный'; spentClass = 'free-lesson'; }
        ...
        if (usedPayment) { const remaining = (usedPayment.lessons_paid || 0) - (usedPayment.lessons_used || 0); spentInfo = `✅ ${formatDate(usedPayment.payment_date)} (ост. ${remaining})`; spentClass = 'paid-lesson'; }
        else { spentInfo = '⚠️ Долг'; spentClass = 'debt-lesson'; }
...
async function renderReportTab(student, lessons, payments) {
  ...
  const startDate = monthAgo.toISOString().split('T')[0], endDate = now.toISOString().split('T')[0];
  return `<div style="padding: 1rem;">
    ...
    <details style="margin-bottom: 1.5rem;"><summary style="cursor: pointer; font-weight: 600; color: var(--text-secondary);"><i class="fas fa-chevron-right"></i> Предпросмотр отчёта (развернуть)</summary>
...
async function renderReminderTab(student, lessons) {
  ...
  return `
    <div style="padding: 1rem;">
      ...
      <p><strong>Осталось уроков:</strong> <span style="color: #d32f2f; font-weight: bold;">1</span></p>
      <div class="form-group"><label>Ближайшие темы для изучения (можно редактировать):</label><input type="text" id="nextTopics" placeholder="Например: Present Perfect, неправильные глаголы" style="width: 100%;"></div>
      <div class="form-group"><label>Сообщение родителю:</label><textarea id="reminderMessage" rows="6" style="width: 100%;">Здравствуйте! У ${student.child_name} остался всего 1 оплаченный урок. Мы провели уже ${completedLessons.length} занятий, изучили темы: ${topics || 'различные темы'}. Чтобы не прерывать прогресс, пожалуйста, оплатите следующие занятия. В ближайшее время планируем изучать: [укажите темы]. Спасибо!</textarea></div>
      <div style="display: flex; gap: 0.5rem; margin-top: 1rem;">
        <button class="btn btn-primary" id="generateAIMessageBtn"><i class="fas fa-robot"></i> ✨ Сгенерировать с ИИ</button>
        <button class="btn btn-success" id="copyReminderBtn"><i class="fas fa-copy"></i> Копировать</button>
      </div>
    </div>
  `;
}
...
async function showPaymentForm(studentId, parentModal) {
  ...
  modal.innerHTML = `
    <div class="modal-card"><h3>Добавить платёж</h3><form id="paymentForm">
  ...
  modal.querySelector('#paymentForm').addEventListener('submit', async (e) => {
    ...
    const amount = modal.querySelector('#paymentAmount').value, lessons = modal.querySelector('#paymentLessons').value, periodStart = modal.querySelector('#periodStart').value, periodEnd = modal.querySelector('#periodEnd').value;
    if (!amount && !lessons && !periodStart) { errorDiv.textContent = 'Укажите сумму, уроки или период'; return; }
    const paymentData = {
      teacher_id: getCurrentUser().id, student_id: studentId,
      payment_date: modal.querySelector('#paymentDate').value,
      amount: amount || null, lessons_paid: lessons ? parseInt(lessons) : null,
...
async function showPaymentReminderModal(student, lessons) {
  const completedLessons = lessons.filter(l => l.status === 'completed' || l.attended === true);
  const topics = completedLessons.slice(-5).map(l => l.topic || 'без темы').join(', ');
  const modal = document.createElement('div');
  modal.className = 'modal payment-reminder-modal';
  modal.innerHTML = `
    <div class="modal-card" style="max-width: 600px;">
      <div class="modal-header"><h3>📨 Напоминание об оплате</h3><button class="close-modal">&times;</button></div>
      <div class="modal-body">
        <p><strong>Ученик:</strong> ${student.child_name}</p>
        <p><strong>Проведено уроков:</strong> ${completedLessons.length}</p>
        <p><strong>Последние темы:</strong> ${topics || '—'}</p>
        <div class="form-group"><label>Ближайшие темы для изучения:</label><input type="text" id="nextTopics" placeholder="Например: Present Perfect" style="width: 100%;"></div>
        <div class="form-group"><label>Сообщение родителю:</label><textarea id="reminderMessage" rows="6" style="width: 100%;">Здравствуйте! У ${student.child_name} остался всего 1 оплаченный урок. Мы провели уже ${completedLessons.length} занятий, изучили темы: ${topics || 'различные темы'}. Чтобы не прерывать прогресс, пожалуйста, оплатите следующие занятия. В ближайшее время планируем изучать: [укажите темы]. Спасибо!</textarea></div>
        <div style="display: flex; gap: 0.5rem; margin-top: 1rem;">
          <button class="btn btn-primary" id="generateAIMessageBtn"><i class="fas fa-robot"></i> ✨ Сгенерировать с ИИ</button>
          <button class="btn btn-success" id="copyReminderBtn"><i class="fas fa-copy"></i> Копировать</button>
...
async function syncUnlinkedLessons(studentId, lessons) {
  ...
  for (const lesson of unlinkedCompletedLessons) {
    ...
    if (payment) { await linkLessonToPayment(lesson.id, payment.id); lesson.payment_id = payment.id; }
  }
}